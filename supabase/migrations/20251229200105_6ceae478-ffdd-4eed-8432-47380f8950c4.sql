-- Update existing budget to associate with the correct artist
UPDATE budgets 
SET artist_id = 'bf35084f-4c6b-4b9b-9a06-9cdef338fb1a'
WHERE name LIKE '%M00DITA%' AND artist_id IS NULL;

-- Fix the on_booking_confirmed trigger to use artist_id directly (not profile_id)
...
BEGIN
  ...
  IF NEW.estado = 'confirmado' AND (OLD.estado IS NULL OR OLD.estado <> 'confirmado') THEN
    ...
    BEGIN
      ...
      -- Create budget with artist_id directly from booking (now references artists table)
      ...
      VALUES (
        ...
        'concierto',
        NEW.artist_id,  -- Use artist_id directly, not profile_id
        NEW.fecha,
        NEW.hora,
        NEW.ciudad,
