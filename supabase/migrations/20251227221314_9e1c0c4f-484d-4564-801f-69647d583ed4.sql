-- Update on_booking_confirmed to pre-populate budget with format crew fees
      ...
      BEGIN
        ...
        IF v_booking_product_id IS NOT NULL AND v_budget_id IS NOT NULL THEN
          ...
          SELECT 
            ...
            COALESCE(
              bpc.role_label,
              -- Try to get name from profiles (workspace members - user_id)
              (SELECT COALESCE(p.stage_name, p.full_name) FROM public.profiles p WHERE p.user_id = bpc.member_id),
              -- Try to get name from contacts
              (SELECT COALESCE(c.stage_name, c.name) FROM public.contacts c WHERE c.id = bpc.member_id),
              'Miembro del equipo'
            ) as name,
            1 as quantity,
            CASE 
            ...
            END as unit_price,
            0 as iva_percentage,
            15 as irpf_percentage,  -- Default IRPF for crew
