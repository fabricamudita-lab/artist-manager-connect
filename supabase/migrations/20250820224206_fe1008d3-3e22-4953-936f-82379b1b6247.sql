...
-- Add due anchors to template items with proper casting
UPDATE public.checklist_template_items SET
  due_anchor = CASE 
    WHEN task LIKE '%venue%' OR task LIKE '%Book%' THEN 'SHOW_DAY'::public.due_anchor
    WHEN task LIKE '%transportation%' OR task LIKE '%accommodation%' THEN 'SHOW_DAY'::public.due_anchor
    WHEN task LIKE '%Sound check%' OR task LIKE '%rehearsal%' THEN 'SHOW_DAY'::public.due_anchor
    WHEN task LIKE '%Script%' OR task LIKE '%storyboard%' THEN 'SHOOT_DAY'::public.due_anchor
    WHEN task LIKE '%Location%' OR task LIKE '%permits%' THEN 'SHOOT_DAY'::public.due_anchor
    WHEN task LIKE '%shooting%' THEN 'SHOOT_DAY'::public.due_anchor
    WHEN task LIKE '%press kit%' THEN 'PRESS_LAUNCH'::public.due_anchor
    WHEN task LIKE '%media outlets%' THEN 'PRESS_LAUNCH'::public.due_anchor
    WHEN task LIKE '%press campaign%' THEN 'PRESS_LAUNCH'::public.due_anchor
    WHEN task LIKE '%Master%' OR task LIKE '%finalize%' THEN 'RELEASE_DAY'::public.due_anchor
    WHEN task LIKE '%streaming%' THEN 'RELEASE_DAY'::public.due_anchor
    WHEN task LIKE '%Social media%' THEN 'RELEASE_DAY'::public.due_anchor
    WHEN task LIKE '%agenda%' THEN 'MEETING_DAY'::public.due_anchor
    WHEN task LIKE '%meeting notes%' THEN 'MEETING_DAY'::public.due_anchor
    WHEN task LIKE '%Review%' OR task LIKE '%approve%' THEN 'PAYRUN_DAY'::public.due_anchor
    WHEN task LIKE '%Process%' OR task LIKE '%approved payments%' THEN 'PAYRUN_DAY'::public.due_anchor
    WHEN task LIKE '%confirmations%' THEN 'PAYRUN_DAY'::public.due_anchor
    ELSE NULL
  END,
