...
BEGIN
  -- 1. Duplicate the booking itself (reset to 'interes')
  ...
  -- 2. Duplicate budgets + budget_items
  FOR v_old_budget_id IN
    SELECT b.id FROM budgets b
    WHERE b.artist_id = v_booking_row.artist_id
  ...
  LOOP
    INSERT INTO budgets (
      name, type, artist_id, event_date, event_time, city, venue, fee,
    ...
    SELECT
      'COPIA - ' || name, type, artist_id, event_date, event_time, city, venue, fee,
      formato, template_id, p_user_id, 'borrador', country,
      capacidad, condiciones, expense_budget, festival_ciclo,
      internal_notes, invitaciones, oferta, project_id,
      NULL, NULL, NULL
  ...
  -- 3a. Duplicate roadmaps with direct booking_id (legacy) + link in junction table
  FOR v_old_roadmap_id IN
    SELECT id FROM tour_roadmaps WHERE booking_id = p_booking_id
  LOOP
    INSERT INTO tour_roadmaps (
      name, artist_id, booking_id, created_by, start_date, end_date,
      promoter, status
    )
    SELECT
      'COPIA - ' || name, artist_id, v_new_booking_id, p_user_id,
      start_date, end_date, promoter, status
    FROM tour_roadmaps WHERE id = v_old_roadmap_id
    RETURNING id INTO v_new_roadmap_id;

    INSERT INTO tour_roadmap_blocks (
      roadmap_id, block_type, data, sort_order
    )
    SELECT
      v_new_roadmap_id, block_type, data, sort_order
    FROM tour_roadmap_blocks WHERE roadmap_id = v_old_roadmap_id;

    -- Also link via junction table for consistency
    INSERT INTO tour_roadmap_bookings (roadmap_id, booking_id, sort_order)
    VALUES (v_new_roadmap_id, v_new_booking_id, 0);
  END LOOP;

  -- 3b. Duplicate roadmaps linked via junction table (excluding legacy ones already processed)
  FOR v_old_roadmap_id IN
  ...
  LOOP
    INSERT INTO tour_roadmaps (
      name, artist_id, created_by, start_date, end_date,
      promoter, status
    )
    SELECT
      'COPIA - ' || name, artist_id, p_user_id,
      start_date, end_date, promoter, status
    FROM tour_roadmaps WHERE id = v_old_roadmap_id
    RETURNING id INTO v_new_roadmap_id;
    ...
    -- Link new roadmap to new booking via junction table
    INSERT INTO tour_roadmap_bookings (roadmap_id, booking_id, sort_order)
    VALUES (v_new_roadmap_id, v_new_booking_id, 0);
  END LOOP;
  ...
  INSERT INTO booking_documents (
    booking_id, document_type, file_name, file_type, file_url,
    content, status, created_by
  )
  SELECT
    v_new_booking_id, document_type, file_name, file_type, file_url,
    content, 'draft', p_user_id
  FROM booking_documents WHERE booking_id = p_booking_id;

  -- 5. Duplicate booking expenses
  INSERT INTO booking_expenses (
    booking_id, description, amount, category, handler, payer,
    iva_percentage, created_by
  )
  SELECT
    v_new_booking_id, description, amount, category, handler, payer,
    iva_percentage, p_user_id
  FROM booking_expenses WHERE booking_id = p_booking_id;

  -- 6. Duplicate booking itinerary
  INSERT INTO booking_itinerary (
    booking_id, item_type, title, description, start_time, end_time,
    location, cost, handler, payer, sort_order, created_by
  )
  SELECT
    v_new_booking_id, item_type, title, description, start_time, end_time,
    location, cost, handler, payer, sort_order, p_user_id
