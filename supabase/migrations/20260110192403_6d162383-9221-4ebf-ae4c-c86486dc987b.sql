-- Fix handle_booking_negotiation(): remove stray reference to alias bpc inside INSERT
-- and make contact join/cast safe when member_id is not a UUID.
...
BEGIN
  ...
  IF NEW.phase = 'negociacion' AND (OLD.phase IS NULL OR OLD.phase != 'negociacion') THEN
    ...
    IF NEW.formato IS NOT NULL AND NEW.artist_id IS NOT NULL THEN
      ...
      -- Add crew members as budget items (contacts only; cast member_id safely)
